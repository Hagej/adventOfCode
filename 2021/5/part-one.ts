...
async function main() {
  ...
  for (const line of lines) {
    if (line.from[0] === line.to[0]) {
      const x = line.from[0];
      for (
        let y = Math.min(line.from[1], line.to[1]);
        y <= Math.max(line.from[1], line.to[1]);
        y++
      ) {
        lineVals[y][x] += 1;
      }
      ...
      const y = line.from[1];
      for (
        let x = Math.min(line.from[0], line.to[0]);
        x <= Math.max(line.from[0], line.to[0]);
        x++
      ) {
        lineVals[y][x] += 1;
      }
    } else if (
      Math.abs(line.from[0] - line.to[0]) ===
      Math.abs(line.from[1] - line.to[1])
    ) {
      ...
      let x = 0,
        y = 0;
      for (
        x = line.from[0], y = line.from[1];
        x !== line.to[0];
        x += Math.sign(xdiff), y += Math.sign(ydiff)
      ) {
        lineVals[y][x] += 1;
      }
      lineVals[y][x] += 1;

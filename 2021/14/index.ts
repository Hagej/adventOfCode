...
export function one(inputFile: string) {
  ...
  const pairs: Array<[string, string]> = data
    .split("\n")
...
export function two(inputFile: string) {
  ...
  for (let i = 0; i < 40; i++) {
    const newTemplate: Record<string, number> = Object.keys(rulePairs).reduce(
      (prev, p) => ({ ...prev, [p]: 0 }),
      {}
    );
...
function insertValue(template: string, pair: [string, string]) {
  ...
  return `${template.substring(0, i + 1)}${pair[1]}${template.substr(
    i + 1,
    template.length - i + 1
  )}`;
}

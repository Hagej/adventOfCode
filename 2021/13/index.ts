...
function subMat(mat: number[][], from: [number, number], to: [number, number]): number[][] {
	const sub: number[][] = []
...
export async function one(inputFile: string) {
	...
	const f = folds[0]
	if (f[0] === "x") {
		let sub = subMat(mat, [(f[1] as number) + 1, 0], [mat[0].length, mat.length])
		...
		let sub = subMat(mat, [0, (f[1] as number) + 1], [mat[0].length, mat.length])
...
function makeEqualLength(a: number[][], b: number[][], axis: "x" | "y"): number[][] {
	let result = [...a]
	if (axis === "x") {
		result = result.map((s) => {
			const row = [...s]
